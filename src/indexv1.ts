...
let selectedTodoListTask: string = JSON.parse(
  localStorage.getItem(LOCAL_STORAGE_SELECTED_TODO)!
)
let todosList: List[] = [...JSON.parse(dbAllTodos)]
...
// set selectedTodoList and rerender
...
const renderTodoList = (parent: HTMLUListElement, list: List) => {
  ...
  LiElement.innerHTML = `
    ...
    `
  //data-list-todo-id
  LiElement.dataset.listTodoId = list.id

  if (list.id === selectedTodoListTask) {
    LiElement.setAttribute('active-list', '')
  }
  parent.appendChild(LiElement)
}

const saveToLocalStorage = (todos: List[]) => {
...
const clearUpcompletedTasks = () => {
  todosList.find(list => {
    ...
    const done = list.tasks?.filter(task => task.completed === false)
    list.tasks = done
